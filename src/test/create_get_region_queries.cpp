...
void print_help(char * argv0) {
    printf("Usage: %s <k3_tree_file> <query_file_output> <n_queries> <k3_tree_type>\n", argv0);
}

template <typename k3_tree_type, typename t_return_type, uint8_t t_precision=0>
void create_get_region_queries(std::string k3_tree_filename, std::string query_filename, size_t n_queries) {
    //************************//
    // Load structure         //
    //************************//
    ...
    t_return_type i_x, i_y, i_z, e_x, e_y, e_z;
    t_return_type min_x = k3_tree.get_min_size_x(),
            min_y = k3_tree.get_min_size_y(),
            min_z = k3_tree.get_min_size_z(),
            max_x = k3_tree.get_max_size_x(),
            max_y = k3_tree.get_max_size_y(),
            max_z = k3_tree.get_max_size_z();
...
//        i_x = min_x + (rand() % (int)((max_x - min_x)-1));
//        i_y = min_y + (rand() % (int)((max_y - min_y)-1));
//        i_z = min_z + (rand() % (int)((max_z - min_z)-1));
//        e_x = i_x + (rand() % (int)((max_x - i_x)-1));
//        e_y = i_y + (rand() % (int)((max_y - i_y)-1));
//        e_z = i_z + (rand() % (int)((max_z - i_z)-1));
        ...
        query_file << std::fixed << std::setprecision(t_precision) << i_x << " " << i_y << " " << i_z;
        query_file << " ";
        query_file << std::fixed << std::setprecision(t_precision) << e_x << " " << e_y << " " << e_z;
...
int main(int argc, char **argv) {
    ...
    std::cout << "Creating " << n_queries << " 'get_region' queries for the k3-tree: " << k3_tree_filename << " (type: " << k3_tree_type << ")" << std::endl;


    switch (k3_tree_type) {
        case sdsl::K3_TREE_TYPE_BASIC:
            create_get_region_queries<sdsl::k3_tree<>, sdsl::k3_tree_base<>::pos_type, 0>(k3_tree_filename, query_filename, n_queries);
            break;
        case sdsl::K3_TREE_TYPE_POINTS:
            create_get_region_queries<sdsl::k3_tree_points<>, sdsl::k3_tree_base<>::pos_type, 0>(k3_tree_filename, query_filename, n_queries);
            break;
        case sdsl::K3_TREE_TYPE_LEVEL:
            create_get_region_queries<sdsl::k3_tree_level<>, sdsl::k3_tree_base<>::pos_type, 0>(k3_tree_filename, query_filename, n_queries);
            break;
        case sdsl::K3_TREE_TYPE_LIDAR_POINTS:
            create_get_region_queries<sdsl::k3_tree_LIDAR_points<>, double, 2>(k3_tree_filename, query_filename, n_queries);
            break;
        default:
            print_help(argv[0]);
    ...
    std::cout << "Created " << n_queries << " 'get_region' queries and stored in " << query_filename << " file " << std::endl;
}
