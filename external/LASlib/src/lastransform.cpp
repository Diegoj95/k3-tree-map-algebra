  ...
  for (i = 1; i < argc; i++)
  {
    ...
    else if (strncmp(argv[i],"-translate_", 11) == 0)
    {
      ...
      else if (strcmp(argv[i],"-translate_xyz") == 0)
      {
        if ((i+3) >= argc)
        {
          fprintf(stderr,"ERROR: '%s' needs 3 arguments: k_offset_x k_offset_y k_offset_z\n", argv[i]);
          return FALSE;
        }
        F64 offset_x;
        if (sscanf(argv[i+1], "%lf", &offset_x) != 1)
        {
          fprintf(stderr,"ERROR: '%s' needs 3 arguments: k_offset_x k_offset_y k_offset_z but '%s' is no valid number\n", argv[i], argv[i+1]);
          return FALSE;
        }
        F64 offset_y;
