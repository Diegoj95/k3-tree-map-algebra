...
class louds_node
{
    ...
    public:
        ...
        void set_values(size_type f_nr=0, size_type f_pos=0) {
            this->m_nr = f_nr;
            this->m_pos = f_pos;
        }
};
...
class louds_tree
{
    ...
    public:
        ...
        louds_tree():m_bv(), m_bv_select1(), m_bv_select0(), bv(m_bv){};
        ...
        void construct(bit_vec_t tmp_bv) {
            m_bv = bit_vector_type(std::move(tmp_bv));
            util::init_support(m_bv_select1, &m_bv);
            util::init_support(m_bv_select0, &m_bv);
        }
        ...
        node_type parent(const node_type& v)const {
            ...
            return node_type(parent_nr, m_bv_select0(parent_nr+1));
        }

        //! Returns an unique id for each node in [0..size()-1]struc
        ...
        size_type serialize(std::ostream& out, structure_tree_node* v=nullptr, std::string name="")const {
            ...
            written_bytes+=m_bv_select1(out, child, "select1");
            written_bytes+=m_bv_select0(out, child, "select0");
