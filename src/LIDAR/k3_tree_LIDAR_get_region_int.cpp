...
void run_get_region_int(std::string k3_tree_filename, std::string query_filename, bool check) {
    //************************//
    // Load structure         //
    //************************//
    ...
    while (!query_file.eof() && query_file.good()) {
        ...
        std::cout << "Query " << n_queries << ": (" << i_x << ", " << i_y << ", " << i_z << ") -- ("
                  << e_x << ", " << e_y << ", " << e_z << ") -- Intensity: " << i_i << " - " << e_i;

        n = k3_tree.get_region_int_attr(i_x, i_y, i_z, e_x, e_y, e_z,
                                                i_i, e_i, result);
        n_points += n;
        std::cout << " = " << n << std::endl;
        if (check) {
...
int main(int argc, char **argv) {
    ...
    switch (k3_tree_type) {
        case sdsl::K3_TREE_TYPE_LIDAR_POINTS:
            run_get_region_int<sdsl::k3_tree_LIDAR_points<>>(k3_tree_filename, query_filename, check);
            break;
        default:
            print_help(argv[0]);
